...
export const createSubmission = (submission) => {
  ...
  return userSubmissions.doc(submission.userId).collection("submissions").add(submission);
};



export const getAllSubmissions = async () => { 
  const query = await userSubmissions.doc(firebaseAuthentication.currentUser.uid).collection("submissions").where("userId", "==", firebaseAuthentication.currentUser.uid)
    .get();
